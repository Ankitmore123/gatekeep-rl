...
class TokenBucketLimiter:
    def __init__(self,redis_client:Redis):
        ...
        local time_elapssed = math.max(0,now - last_updated)
        local tokens_to_add = time_elapsed * refill rate
        local new_tokens = max(capcity, current_tokens+ tokens_to_add)
        ...
        if new_tokens >= 1 then
            ...
            -- Save the replenished tokens even if blocked, so progress isn't lost
            redis.call('HMSET', key, 'tokens', new_tokens, 'last_updated', now)
            return 0
        """
